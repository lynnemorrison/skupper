...
func TestCmdDebug_Run(t *testing.T) {
	...
	for _, test := range testTable {
		...
		assert.Check(t, err == nil)
		err = command.routerAccessHandler.WriteFile(path, "my-routerAccess.yaml", content, common.RouterAccesses)
		assert.Check(t, err == nil)
		...
		assert.Check(t, err == nil)
		err = command.certificateHandler.WriteFile(path, "my-certificate.yaml", content, common.Certificates)
		assert.Check(t, err == nil)
		//defer command.certificateHandler.Delete("my-certificate")

		content, err = command.secretHandler.EncodeToYaml(secretResource)
		assert.Check(t, err == nil)
		err = command.secretHandler.WriteFile(path, "my-secret.yaml", content, common.Secrets)
		assert.Check(t, err == nil)
